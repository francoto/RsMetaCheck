...
```bash
uv run rsmetacheck --input https://github.com/tidyverse/tidyverse
```

#### Analyze Multiple Repositories from a JSON File

```bash
uv run rsmetacheck --input repositories.json
```
...
```bash
uv run rsmetacheck --skip-somef --input somef_outputs/*.json
```

Or for multiple paths:

```bash
uv run rsmetacheck --skip-somef --input my_somef_outputs_1/*.json my_somef_outputs_2/*.json
```
...
```bash
uv run rsmetacheck --input https://github.com/tidyverse/tidyverse --verbose
```
